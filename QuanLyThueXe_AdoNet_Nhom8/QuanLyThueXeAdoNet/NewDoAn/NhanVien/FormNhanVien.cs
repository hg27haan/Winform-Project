...
namespace NewDoAn
{
    public partial class FormNhanVien : MaterialForm
    {
        ...
        private bool KiemTraCuoiThangSau5h30()
        {
        ...
        private void materialTabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                if (KiemTraCuoiThangSau5h30() == true)
                {
            ...
            {
                if (DateTime.Now.Month == 12 && KiemTraCuoiThangSau5h30() == true)
                {
        ...
        private void LoadTrendingThueXe()
        {
            ...
            gvTrending.DataSource = tTXDAO.TrendingThueXe();
        ...
        private bool KiemTraXeConDatThue(string str)
        {
            if (dBC.KiemTraMaXe("ChiTietDatThue", str) == true)
            {
                return true;
            }
            return false;
        ...
        {
            if (KiemTraXeConDatThue(suaMaXe) == true)
            {
                MessageBox.Show("Xe này còn đang trong danh sách Đặt/Thuê nên không thể chỉnh sửa, phải đợi xe không trong trạng thái được " +
                    "Đặt/Thuê", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }    
            else
            {
        ...
        private void LoadVaDoiTenDanhSachTaiKhoanKhachHang()
        {
            ...
            gvDanhSachTaiKhoanKhachHang.DataSource = tKDAO.LayDanhSachTaiKhoanKhachHang();
        ...
        {
            if (dBC.KiemTraKhachHangConDatThue(tTCN) == true)
            {
                MessageBox.Show("Khách hàng này còn đang Đặt/Thuê xe nên không thể chỉnh sửa, phải đợi vị khách này hủy/trả hết xe đang " +
                    "Đặt/Thuê mới có thể thực hiện", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
                ...
                {
                    tKDAO.SuaThongTinDangNhap(txtHoVaTen.Text, txtCMND.Text, txtSDT.Text, txtDiaChi.Text, tenDangNhap,
                        txtMatKhau.Text);
                    LoadVaDoiTenDanhSachTaiKhoanKhachHang();
        ...
        private void DanhSachKiemTraXeDatThue()
        {
            ...
            gvKiemTraXeDatThue.DataSource = tTBDAO.KiemTraXeDatThue(txtTenKhachHang.Text, txtCMNDKhachHang.Text);
        ...
        private void LoadVaDoiTenDanhSachXePhaiTraHomNay()
        {
            ...
            gvDanhSachXeTraHomNay.DataSource = tTBDAO.LocDanhSachXePhaiTraHomNay(DateTime.Now.Date);
        ...
        private void LoadVaDoiTenDanhSachDoanhThuHomNay()
        {
            ...
            gvDoanhThuHomNay.DataSource = tTBDAO.LayDoanhThuHomNay(DateTime.Now.Date);
        ...
        {
            dTDAO.ThemVaoDoanhThuThang(DateTime.Now.Date.ToString("dd/MM/yyyy"), tongTien);
        ...
        private void LoadBieuDoDoanhThuThang()
        {
            ...
            string str = "DoanhThuThang";
            dBC.LayDataBaseDoanhThuThangNam(str, ref lstNgay, ref lstDoanhThu);

            for (int i = 0; i < lstNgay.Count; i++)
            {
                chartDBC.Series["DoanhThuThang"].Points.AddXY(lstNgay[i], double.Parse(lstDoanhThu[i]));
                chartDBC.Series["DoanhThuThang"].Points[i].Label = int.Parse(lstDoanhThu[i]).ToString("N0",
        ...
        private void btnTongKetVaXuatRaJPG_Click(object sender, EventArgs e)
        {
            ...
            {
                LuuJPG(chartDBC);
                string doanhThuMotThang = "";
                dBC.TinhDoanhThuThang(ref doanhThuMotThang);
        ...
        private void LoadBieuDoDoanhThuNam()
        {
            ...
            List<string> lstDoanhThu = new List<string>();
            string str = "DoanhThuNam";
            dBC.LayDataBaseDoanhThuThangNam(str, ref lstThang, ref lstDoanhThu);

            for (int i = 0; i < lstThang.Count; i++)
            {
                chartDoanhThuNam.Series["DoanhThuNam"].Points.AddXY(lstThang[i], double.Parse(lstDoanhThu[i]));
                chartDoanhThuNam.Series["DoanhThuNam"].Points[i].Label = int.Parse(lstDoanhThu[i]).ToString("N0",
                    CultureInfo.CreateSpecificCulture("vi-VN"));
        ...
        private void btnXacNhanThem_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dBC.KiemTraMaXe("DanhSachXeHienTai",txtThemMaXe.Text) == false)
                {
                    ...
                    {
                        MessageBox.Show("Kiểm tra lại phần Giá Thuê", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }    
                }
                else
                {
        ...
        private void LoadVaDoiTenMaGiamGia()
        {
            ...
            gvMaGiamGia.DataSource = mGGDAO.LayDanhSach();
        ...
        private void btnThemMaGiamGia_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dBC.KiemTraMaGiamGia(txtCode.Text) == true)
                {
        ...
        private void LoadVaDoiTenTKNhanVien()
        {
            ...
            gvTaiKhoanNhanVien.DataSource = tKDAO.LoadDanhSachTKNhanVien();
